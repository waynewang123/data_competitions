...
def train_fn(data_loader,model,optimizer,device,scheduler):
    ...
    for d in  data_loader:
        ...
        #from current batch also extract targets
        ids = d['ids']
...
def eval_fn(data_loader,model,device):
    ...
    with torch.no_grad():
        #this part is the same as training fuction
        #except for the fact that there is no zero_grad of optimizer and there is no loss calculation or scheduler steps
        for d in data_loader:
            ids = d['ids']
            token_type_ids = d['token_type_ids']
            mask = d['mask']
